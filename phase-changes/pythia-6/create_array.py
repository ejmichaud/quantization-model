...
results_dir = '/om/user/ericjm/results/phase-changes/pythia-6/'
...
# checkpoint_files = [f for f in os.listdir('/om/user/ericjm/results/phase-changes/pythia-6') if f != 'logs']
# checkpoint_files = sorted(checkpoint_files, key=lambda c: int(c.split('.pt')[0]))
# print(checkpoint_files[:20])
# example_data = torch.load("/om/user/ericjm/results/phase-changes/pythia-6/step1000.pt")
...
np.save(os.path.join(results_dir, 'pythia-6.npy'), timeseries)
