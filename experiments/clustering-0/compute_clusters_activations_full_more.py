...
idxs, C, C_abs = torch.load("/om/user/ericjm/results/the-everything-machine/clustering-0/activation_full_more.pt")
C = C.cpu().numpy()
C_abs = C_abs.cpu().numpy()
C = 1 - np.arccos(C) / np.pi
# convert NaN values of C to 1.0
C[np.isnan(C)] = 1.0
...
for n_clusters in tqdm(CLUSTER_COUNTS):
    clusters_labels = sklearn.cluster.SpectralClustering(n_clusters=n_clusters, affinity='precomputed', n_init=5).fit_predict(C)
    clusters_labels_abs = sklearn.cluster.SpectralClustering(n_clusters=n_clusters, affinity='precomputed', n_init=5).fit_predict(C_abs)
    results[n_clusters] = (clusters_labels.tolist(), clusters_labels_abs.tolist())

    with open("/om/user/ericjm/results/the-everything-machine/clustering-0/clusters_activations_full_more.pkl", "wb") as f:
